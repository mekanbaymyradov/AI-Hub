...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["user.id"],
            name=op.f("attachment_user_id_fkey"),
            ondelete="cascade",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("attachment_pkey")),
    )
    op.create_index(
        op.f("attachment_message_id_idx"), "attachment", ["message_id"], unique=False
    )
    op.create_index(
        op.f("attachment_user_id_idx"), "attachment", ["user_id"], unique=False
    )
